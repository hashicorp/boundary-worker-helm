#!/bin/bash
# ============================================================================
# AKS Acceptance Test — Boundary Worker Helm Chart
#
# Validates that the Boundary Worker Helm chart is correctly installed and
# functioning on an Azure AKS cluster. Mirrors the EKS acceptance workflow
# but with AKS-specific validations (Azure LB, managed-csi PVCs, etc.).
#
# Required env vars:
#   AZURE_RESOURCE_GROUP    - Azure resource group containing the AKS cluster
...
#   WAIT_TIMEOUT            - Readiness wait in seconds (default: 300)
#   LB_TIMEOUT              - Azure LB IP provisioning wait in seconds (default: 300)
# ============================================================================
...
# AKS context is the cluster name after az aks get-credentials
...
record_pass "Azure credentials are valid"
...
az aks get-credentials \
    ...
    || fail "Failed to get credentials for '${AKS_CLUSTER_NAME}'. Verify the cluster exists and the resource group is correct."
...
if kubectl get daemonset csi-azuredisk-node \
    ...
    record_pass "Azure Disk CSI Driver DaemonSet is present"
else
    warn "csi-azuredisk-node DaemonSet not found — driver may still be initialising"
fi

# Azure Disk CSI Controller
if kubectl get deployment csi-azuredisk-controller \
        -n kube-system --context "${AKS_CONTEXT}" >/dev/null 2>&1; then
    record_pass "Azure Disk CSI Controller deployment is present"
else
    warn "csi-azuredisk-controller not found"
fi

# managed-csi StorageClass
if kubectl get storageclass managed-csi \
        --context "${AKS_CONTEXT}" >/dev/null 2>&1; then
    record_pass "managed-csi StorageClass is available"
else
...
if [ "${PVC_COUNT}" -gt 0 ]; then
    ...
    # Verify managed-csi storage class is used
    MCSI_PVCS=$(kubectl get pvc -n "${NAMESPACE}" \
        --context "${AKS_CONTEXT}" \
        -o jsonpath='{range .items[*]}{.spec.storageClassName}{"\n"}{end}' 2>/dev/null \
        | grep -c "^managed-csi$" || true)
    [ "${MCSI_PVCS}" -gt 0 ] \
        && record_pass "${MCSI_PVCS} PVC(s) use managed-csi storage class" \
        || warn "No PVCs found using managed-csi — check storageClass configuration"
else
...
while [ "${ELAPSED}" -lt "${LB_TIMEOUT}" ]; do
    ...
    info "Waiting for Azure Load Balancer IP... (${ELAPSED}s elapsed)"
    sleep "${INTERVAL}"
...
if [ -n "${LB_IP}" ]; then
    ...
    LB_SVC_TYPE=$(kubectl get svc \
        -n "${NAMESPACE}" \
        --context "${AKS_CONTEXT}" \
        -l app.kubernetes.io/name=boundary-worker \
        -o jsonpath='{.items[?(@.spec.type=="LoadBalancer")].spec.type}' \
        2>/dev/null || true)
    [ "${LB_SVC_TYPE}" = "LoadBalancer" ] \
        && record_pass "Service type is LoadBalancer" \
        || warn "LoadBalancer service type not confirmed"

    # Verify proxy port 9202 is exposed
    PROXY_PORT=$(kubectl get svc \
        -n "${NAMESPACE}" \
        --context "${AKS_CONTEXT}" \
        -l app.kubernetes.io/name=boundary-worker \
        ...
        2>/dev/null || true)
    [ "${PROXY_PORT}" = "9202" ] \
        && record_pass "Proxy port 9202 is exposed on the LoadBalancer service" \
        || warn "Proxy port check: found '${PROXY_PORT}' (expected 9202)"
else
...
if [ "${TESTS_FAILED}" -gt 0 ]; then
    ...
    echo ""
    echo "✅ All AKS acceptance tests passed!"
fi
